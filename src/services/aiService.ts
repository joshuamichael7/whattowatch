...
async function findBestMatch(aiItem: any, omdbResults: any[]): Promise<any> {
  ...
  if (omdbResults.length === 1)
    return await getFullDetails(omdbResults[0].imdbID);

  // If we have a year, try to match by year first
  if (aiItem.year) {
    const yearMatch = omdbResults.find((result) => {
      // Handle year ranges like "2019–2022" in TV shows
      const resultYear = result.Year.split("–")[0];
      return resultYear === aiItem.year.toString();
    });

    if (yearMatch) return await getFullDetails(yearMatch.imdbID);
  }
